...
public class RESTHandler{
    ...
    /*
        ...
        of the REQUEST Parmaters 
        and return it to parent node in hashmap format.
        
    */
    
     public Map<String,String> IdentifyJsonNodes_request(JSONObject jObj,String key,String parentPath,Map<String,String> jsonNodes, int count){
		 JSONObject j;
		 if (jObj.get(key) instanceof JSONObject){
			 ...
			 if (j.keySet().size()>0){
					 ...
					 jsonNodes = IdentifyJsonNodes_request(j,key1,parentPath,jsonNodes,0);
					 }
				 }
		 }
			 ...
			 while(itr.hasNext()){
		            ...
		            if (featureJsonObj.keySet().size()>0){
		            	int ct =0;
						 ...
						 jsonNodes = IdentifyJsonNodes_request(featureJsonObj,key1,parentPath,jsonNodes,ct);
						 }
						 ct++;
					 }
...
return jsonNodes;
    ...
    */
    ...
    public ArrayList<String> executeRESTServiceCall(String jsonCompareOption, String url, String inputObject,String expectedOutput,ArrayList<String> ignoreParams,
        ...
        switch(jsonCompareOption){
            ...
            case "JsonExactMatch" :
                    {
                     ...
                     //   new DatabaseHandler().insertDataToLogTable(test_case_name,inputObject,resp,url);
                		...
                		//}
                         returnValues = returnValueFunction(returnTags,jsonNodes);
          ...
          break;
                ...
                case "StringCompare" :
                    {
                        System.out.println("String Compare selected");
                         if(inputObject== null){
                            inputObject="";
                        }
                       if(method_type.equals("get")){
    ...
    public String replaceNewValuesInRequestJson(String jsonRequest,ArrayList<String> tags,ArrayList<String>values,String api_name){
        Map<String,String> updatedRequest = getJsonMap_request(jsonRequest);
        ...
        if(tags.size()>0){
            ...
            for(int i=0;i<tags.size();i++){
                ...
                System.out.println("\""+tags.get(i)+"\":oldValue \""+oldValue+"\"");
                System.out.println("New Value : "+ newValue);
               //int intIndex = jsonRequest.indexOf("\""+tags.get(i)+"\":\""+oldValue+"\"");
               // System.out.println("String Found : "+intIndex);
               jsonRequest = jsonRequest.replaceAll("\\s+","");
                jsonRequest=jsonRequest.replace("\""+tags.get(i)+"\":\""+oldValue+"\"","\""+tags.get(i)+"\":\""+newValue+"\"");
                  System.out.println("Updated Json inside IF: "+jsonRequest);
            }
        }
    ...
    public ArrayList<String> returnValueFunction(ArrayList<String> returnValues,Map<String,String> inputHashMap){
        //System.out.println(" ignore These "+ignoreParams );  returnValues.add(actualValues.get(tag));
        Iterator<Map.Entry<String,String>> it = inputHashMap.entrySet().iterator();
       	ArrayList<String> returnArr = new ArrayList<String>();
        while (it.hasNext()) {
			Map.Entry pair = (Map.Entry)it.next();	
            for(String key: returnValues){
                 //System.out.println(" outsideIF "+key );
                if(pair.toString().contains(key.toString())){
                    //System.out.println(pair.getKey() + " = " + pair.getValue());
                    //it.remove();
                   returnArr.add(pair.getValue().toString()); 
                    //System.out.println(" insideIF "+key );
                }
                    //it.remove(); // avoids a ConcurrentModificationException
            }
     ...
     public Map<String,String> getJsonMap_request(String jsonResponseString){
        //System.out.println("processing json for request:" +jsonResponseString);
        JSONObject jSonResponseObject = new JSONObject(jsonResponseString);
        //System.out.println("converted to Json object");
     	Map <String,String> jsonNodes = new HashMap<String,String>();
        String parentPath="";
      	for (String key : jSonResponseObject.keySet()){
    		jsonNodes=  IdentifyJsonNodes_request(jSonResponseObject, key,parentPath,jsonNodes,0);
    	}
        System.out.println("jsonNodes request: "+ jsonNodes);
         return jsonNodes;
    }
}